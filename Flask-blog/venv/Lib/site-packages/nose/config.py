...
class Config(object):
    ...
    all config keys::
      ...
      self.venv = venv = kw.pop('venv', {})
      self.args = ()
      self.testMatch = re.compile(r'(?:^|[\\b_\\.%s-])[Tt]est' % os.sep)
      self.addPaths = not venv.get('NOSE_NOPATH', False)
      self.configSection = 'nosetests'
      self.debug = venv.get('NOSE_DEBUG')
      self.debugLog = venv.get('NOSE_DEBUG_LOG')
      self.exclude = None
      self.getTestCaseNamesCompat = False
      self.includeExe = venv.get('NOSE_INCLUDE_EXE',
      ...
      self.runOnInit = True
      self.stopOnError = venv.get('NOSE_STOP', False)
      self.stream = sys.stderr
      self.testNames = ()
      self.verbosity = int(venv.get('NOSE_VERBOSE', 1))
      self.where = ()
      self.py3where = ()
    ...
    def __repr__(self):
        ...
        # don't expose venv, could include sensitive info
        d['venv'] = {}
